...
import com.parul.imdbapplication.presentation.viewModel.SecondFragmentViewModel.Companion.NAV_BACK
import com.parul.imdbapplication.presentation.viewModel.SecondFragmentViewModel.Companion.UPDATE_ERROR
import com.parul.imdbapplication.presentation.viewModel.SecondFragmentViewModel.Companion.UPDATE_WEATHER_DATA
...
class SecondFragment : BaseFragment(), SecondFragmentEventListener {
    ...
    override fun onCreateView(
        ...
        handleArguments()
        setupBackPress()
        binding.lifecycleOwner = this // required to add this line if we want two way data binding in xml, otherwise data wont be refreshed when we get API response.
        return with(binding) {
    ...
    private fun updateWeatherDetails() {
        ...
        /*Glide.with(this)
            .load(viewModel.weatherData.value.weatherIconUrl)
            .into(imageview)*/
